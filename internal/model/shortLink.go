...
type ShortLink struct {
	...
	Domain        string    `gorm:"type:nvarchar(50);column:domain;comment:'域名';not null"`
	...
	Uri           string    `gorm:"type:nvarchar(255);column:uri;comment:'生成短链接的uri';not null;index:idx_gid_uri" json:"uri"`
}

// TName 对应的分表表名
func (s ShortLink) TName() string {
	id := hash(s.Gid)
	return fmt.Sprintf("short_link_%d", id%ShortLinkShardingNum)
}
