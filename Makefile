...
graph:
	...
	@cp -f cmd/SnapLink/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=SnapLink SnapLink
	@rm -f main.go SnapLink.gv
...
.PHONY: build
# build SnapLink for linux amd64 binary
build:
	@echo "building 'SnapLink', linux binary file will output to 'cmd/SnapLink'"
	@cd cmd/SnapLink && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
